...
export async function buildMonthSuggestions(input: {
  ...
  try {
    ...
    return parsed
      ...
      // Only channels we actually run (LinkedIn, Instagram, Website). Drop anything
      // else (e.g. a stray TikTok/YouTube idea) rather than mislabelling it.
      .filter((x) => CHANNELS.includes(String(x.channel).toLowerCase()))
      .map((x) => ({
