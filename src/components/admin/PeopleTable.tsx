...
import { setUserRole, setUserTeam, setUserStatus } from "@/lib/admin/actions";
import { ROLES, type Role } from "@/lib/auth/roles";
import type { Person, Team } from "@/lib/admin/types";

/* The People list — assign each teammate a role and team, and activate /
   deactivate them. Founder tier can only be granted by a founder. */
...
export default function PeopleTable({ people: initial, teams, myRole }: { people: Person[]; teams: Team[]; myRole: Role }) {
  ...
  async function changeRole(p: Person, role: Role) {
    ...
    if (!r.ok) {
      patch(p.id, { role: prev });
      flash(r.reason === "founder_only" ? "Only a founder can change the founder tier." : "Couldn't update role.");
    }
  }
  ...
  async function toggleStatus(p: Person) {
    ...
    if (!r.ok) {
      patch(p.id, { status: prev });
      flash(r.reason === "self" ? "You can't deactivate yourself." : r.reason === "founder_only" ? "Only a founder can deactivate a founder." : "Couldn't update.");
    }
  }
    ...
    <div className="sw-card overflow-hidden">
      <div className="flex items-center justify-between gap-3 border-b border-hairline px-5 py-3">
        <div>
          ...
          <p className="text-[11px] text-hint">{people.length} {people.length === 1 ? "person" : "people"} · assign roles, teams and status</p>
        </div>
      ...
      <div className="divide-y divide-hairline">
        ...
        {people.map((p) => (
          <div key={p.id} className="flex flex-wrap items-center gap-x-3 gap-y-2 px-5 py-3">
            ...
            >
              {ROLES.map((r) => (
                <option key={r.key} value={r.key} disabled={(r.key === "founder" || p.role === "founder") && myRole !== "founder"}>
                  {r.label}
                </option>
              ))}
            </select>
            ...
            <select
              value={p.teamId ?? ""}
              onChange={(e) => changeTeam(p, e.target.value || null)}
              className="shrink-0 rounded-[var(--radius-control)] border border-hairline bg-surface px-2 py-1 text-xs text-muted focus:outline-none"
            >
            ...
            <button
              ...
              disabled={p.isSelf}
              className={[
                "shrink-0 rounded-full px-2.5 py-1 text-xs font-medium transition-colors disabled:opacity-40",
                p.status === "deactivated" ? "bg-emerald-500/15 text-emerald-700 hover:brightness-95" : "border border-hairline text-muted hover:text-red-600",
              ].join(" ")}
            >
      ...
      <p className="border-t border-hairline px-5 py-2.5 text-[11px] text-hint">
        Everyone who signs in with Google appears here automatically. Roles decide what each person can see — set that on the Access page.
      </p>
    </div>
  );
}
