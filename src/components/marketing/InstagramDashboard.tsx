...
/* The Instagram channel dashboard — one page per sidebar section. Header and
   Alfred's read are constant; the body swaps by section. Live from the
   Instagram API; honest empties where a metric isn't provided yet. */
...
function SectionHead({ n, title, note, ctx }: { n: string; title: string; note?: string; ctx?: string }) {
  return (
...
export default function InstagramDashboard({ data, section = "overview" }: { data: IgData; section?: string }) {
  ...
  const RecentPosts = (
    <div className="mc-card mc-panel">
      ...
      <div style={{ marginTop: 3 }}>
        {data.media.length ? data.media.slice(0, 8).map((m) => {
  ...
  const body = (() => {
    switch (section) {
      case "content":
        return <section className="mc-zone"><SectionHead n="02" title="Content" note="recent posts & formats" /><div className="mc-split">{RecentPosts}{FormatCard}</div></section>;
      case "analytics":
        return (
          <section className="mc-zone">
            <SectionHead n="02" title="Analytics" note="28-day account metrics" ctx="From Instagram Insights" />
      ...
      case "audience":
        return <section className="mc-zone"><SectionHead n="03" title="Audience" note="growth & demographics" /><div className="mc-g2">{GrowthCard}{DemoCard}</div></section>;
      case "reports":
        return (
          <section className="mc-zone">
            <SectionHead n="04" title="Reports" note="account snapshot" />
            <div className="mc-card mc-panel">
              <div className="mc-ph"><div><h3>Instagram report</h3><p className="mc-sub">Live snapshot of the account</p></div><span className="mc-badge pink">28d</span></div>
              <div className={`mc-kpis ${kClass}`} style={{ marginTop: 10 }}>
                {kpis.map((k) => <KpiCard key={k.label} label={k.label} value={k.value} note={k.note} />)}
              </div>
              <p className="mc-soft" style={{ fontSize: 11, marginTop: 10, lineHeight: 1.5 }}>One-click PDF and emailed reports are coming next — for now this is your live snapshot.</p>
            </div>
          </section>
        );
      case "engagement":
        return <section className="mc-zone"><SectionHead n="" title="Engagement" /><SectionPlaceholder title="Engagement" reason="Comments and DMs are handled in the Engagement inbox, where Alfred drafts replies in your founder voice and you approve every send." cta={<a className="mc-cta sm" href="/dashboard/marketing/engagement">Open Engagement inbox →</a>} /></section>;
      case "stories":
        return <section className="mc-zone"><SectionHead n="" title="Stories" /><SectionPlaceholder title="Stories" reason="Story insights (taps, exits, replies) appear here once you post Stories on @swisswiper — the API reports them per Story." /></section>;
      case "reels":
        return <section className="mc-zone"><SectionHead n="" title="Reels" /><SectionPlaceholder title="Reels" reason="Reels currently count toward Content and Format performance. A dedicated Reels breakdown (plays, watch-through) appears here as you publish more Reels." /></section>;
      case "hashtags":
        return <section className="mc-zone"><SectionHead n="" title="Hashtags" /><SectionPlaceholder title="Hashtags" reason="Per-hashtag reach isn't exposed by the Instagram API for owned posts. If it matters, I can track a chosen set manually and surface them here." /></section>;
      case "competitors":
        return <section className="mc-zone"><SectionHead n="" title="Competitors" /><SectionPlaceholder title="Competitors" reason="Competitor benchmarking needs a source for the accounts you want to track. Tell me which to watch and I'll wire it up." /></section>;
      case "campaigns":
        return <section className="mc-zone"><SectionHead n="" title="Campaigns" /><SectionPlaceholder title="Campaigns" reason="Paid-campaign metrics appear here once you run Instagram/Meta ads and connect the ad account." /></section>;
      default:
        return <section className="mc-zone"><SectionHead n="01" title="Overview" note="account at a glance" ctx="28-day metrics where Instagram provides them" /><div className={`mc-kpis ${kClass}`}>{kpis.map((k) => <KpiCard key={k.label} label={k.label} value={k.value} note={k.note} />)}</div></section>;
    }
  })();
